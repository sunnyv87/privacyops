...
- **Record new consent** — Via API or manual entry
...
3. Click a request to begin processing:
   - **Verify identity** — Confirm the data subject's identity
   ...
   - **Review** — Review collected data before responding
...
2. Ask questions in natural language, for example:
   - "What PII do we store in the customer database?"
   - "Show me our compliance gaps for DPDP 2023"
   - "Draft a breach notification for the incident reported yesterday"
   - "What data retention policies apply to financial records?"
   - "Help me create a DPIA for our new marketing campaign"

> **Note:** The AI Co-Pilot requires an `ANTHROPIC_API_KEY` to be configured.
